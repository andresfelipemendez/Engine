...
private:
    ...
    VkSampleCountFlagBits msaaSaples = VK_SAMPLE_COUNT_1_BIT;
    VkDevice device;
    ...
    void pickPhysicalDevice()
    {
        ...
        for (const auto &device : devices)
        {
            if (isDeviceSuitable(device))
            {
                ...
                msaaSaples = getMaxUsableSampleCount();
                break;
            }
        }
    ...
    void createLogicalDevice()
    {
        ...
        for (uint32_t queueFamily : uniqueQueueFamilies)
        {
            ...
            queueCreateInfo.pQueuePriorities = &queuePriotiy;
    ...
    void createRenderPass()
    {
        ...
        colorAttachment.samples = msaaSaples;
        ...
        depthAttachment.samples = msaaSaples;
        ...
        dependecy.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
        dependecy.srcAccessMask = 0;
        dependecy.dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
        dependecy.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
    ...
    void createGraphicsPipeline()
    {
        ...
        multisampling.rasterizationSamples = msaaSaples;
    ...
    void createColorResources()
    {
        ...
        createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSaples, colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorImage, colorImageMemory);
    ...
    void createDepthResources()
    {
        ...
        createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSaples, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_HEAP_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
    ...
    void createTextureImage()
    {
        ...
        gnerateMipmaps(textureImage, VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mipLevels);
    }

    void gnerateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
    {
    ...
    void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer &buffer, VkDeviceMemory &bufferMemory)
    {
        ...
        {
            throw std::runtime_error("failed to ccreate vertex buffer");
        }
    ...
    {
        for (const auto &avaiableFormat : availableFromats)
        {
            if (avaiableFormat.format == VK_FORMAT_B8G8R8_SRGB && avaiableFormat.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR)
            {
